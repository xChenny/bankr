...
def init_glassdoor():
    ...
    driver = '/home/dchen/Downloads/chromedriver'
...
def searchJobs(jobName, city=None, link=None, browser=None):
    ...
    sleep(1.5)
    location.send_keys(keys.Keys.RETURN)
    ...
    for i in range(25):
        ...
        try:
            ...
            jobPosting = browser.find_elements_by_xpath('//span[@class="green small"]/../../../..')
            sleep(get_pause())
...
def closeModal(browser):
    sleep(get_pause())
    browser.execute_script("document.querySelectorAll('div.xBtn')[0].click()")


###############################################################################    

def searchJobSalaries(jobName, city=None, jobDict = None, link=None, browser=None):
    '''Scrape for job listing'''

    q = raw_input('Shall we scrape? (y/n)')
    
    if q=='y':
        browser.execute_script("document.querySelectorAll('li.col-3.salaries')[0].click()")
        sleep(2)
        job = browser.find_element_by_id("KeywordSearch")
        location = browser.find_element_by_id("LocationSearch")
        sleep(3)
...
#    f = open('/Users/Diego/nltk_data/corpora/stopwords/english')
    words= set(stopwords.words("english"))#f.read().splitlines()
#    f.close()
...
#        pass
    ...
    text = [w for w in text if not w in words]
